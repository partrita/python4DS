...
이 책의 첫 번째 부분을 읽고 나면, 데이터 과학을 수행하기 위한 가장 중요한 도구들의 기초를 이해하게 됩니다. 이제 세부 사항으로 뛰어들 시간입니다. 책의 이 부분에서는 데이터 시각화에 대해 더 깊이 배우고( {ref}`vis-layers`에서), 다양한 종류의 데이터 시각화에 대한 세부 사항들을 본격적으로 다룰 것입니다( {ref}`exploratory-data-analysis` 및 {ref}`communicate-plots`에서). 이 짧은 장에서는 시각화를 만드는 다양한 방법과 시각화의 다양한 목적에 대해 논의합니다.
...
데이터 시각화를 통한 소통 주제에 대한 더 많은 정보는 {ref}`communicate-plots` 장에서 찾을 수 있습니다.
